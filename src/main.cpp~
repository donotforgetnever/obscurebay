...
int nCoinbaseMaturity = 70; // 70 blocks for coins to mature
...
const string strMessageMagic = "obscurebay Signed Message:\n";
...
int64_t GetProofOfWorkReward(int64_t nFees)
{
    ...
    else if(pindexBest->nHeight < 1666)
    {
        nSubsidy = 2000 * COIN;
    }
    else if(pindexBest->nHeight < 2666)
    {
        nSubsidy = 1000 * COIN;
    }
    else if(pindexBest->nHeight < 3666)
    {
        nSubsidy = 800 * COIN;
    }
    else if(pindexBest->nHeight < 4666)
    {
        nSubsidy = 600 * COIN;
    }
    else if(pindexBest->nHeight < 5666)
    {
        nSubsidy = 400 * COIN;
    }
    else if(pindexBest->nHeight < 6666)
    {
        nSubsidy = 200 * COIN;
    }
    else if(pindexBest->nHeight < 7666)
    {
        nSubsidy = 100 * COIN;
    }
    else if(pindexBest->nHeight < 8666)
    {
        nSubsidy = 50 * COIN;
    }
    else if(pindexBest->nHeight < 9666)
    {
        nSubsidy = 100 * COIN;
    }
...
// obscurebay: attempt to generate suitable proof-of-stake
...
bool CheckDiskSpace(uint64_t nAdditionalBytes)
{
    ...
    if (nFreeBytesAvailable < nMinDiskSpace + nAdditionalBytes)
    {
        ...
        printf("*** %s\n", strMessage.c_str());
        uiInterface.ThreadSafeMessageBox(strMessage, "obscurebay", CClientUIInterface::OK | CClientUIInterface::ICON_EXCLAMATION | CClientUIInterface::MODAL);
        StartShutdown();
        return false;
    }
    return true;
...
bool LoadBlockIndex(bool fAllowNew)
{
    ...
    if (mapBlockIndex.empty())
    {
        ...
        const char* pszTimestamp = "obscurebay coin x11";
        CTransaction txNew;
        txNew.nTime = 1421756332;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        ...
        //// debug print
        assert(block.hashMerkleRoot == uint256("0xe71e9a535a1ab71fa6e56fd03be23a867b730fbc211166e401ffc910d16788af"));
        block.print();
